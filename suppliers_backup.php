...
try {
    ...
    // Get all suppliers
    $allSuppliersQuery = "SELECT id, slug, logo, name, description, address, email, phone, created_at FROM suppliers WHERE status = 1 ORDER BY name ASC";
    $allStmt = $pdo->query($allSuppliersQuery);
    $allSuppliers = $allStmt->fetchAll(PDO::FETCH_ASSOC);
    
    // Divide suppliers into groups
    $featuredSuppliers = array_slice($allSuppliers, 0, 4); // First 4 as featured
    $recentSuppliers = $allSuppliers; // All suppliers as recent
    $randomSuppliers = []; // Empty for now
    
    // Get search results with pagination
    if (!empty($search) || !empty($letter)) {
        $searchQuery = "SELECT id, slug, logo, name, description, address, email, phone, created_at FROM suppliers $whereClause ORDER BY name ASC LIMIT :limit OFFSET :offset";
